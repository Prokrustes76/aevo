...
function getContextPhase3() {
  for (let i = 0; i < 3; i++) {
    ...
    elem.setAttribute('class', 'themen hoverBar gradient');
    elem.style = i == 0 ? "text-shadow: none" : "text-shadow: 2px 2px 4px rgba(0, 0, 0, .2)";
    elem.style.width = i == 0 ? 'max-content' : '50%';
    elem.style.margin = '25px auto 45px auto';
    elem.style.pointerEvents= 'none';
    elem.style.textAlign = 'left';
    elem.style.fontSize = '1.2em';
    elem.style.background = i == 0 ? 'darkblue' : 'lightblue';
    elem.style.color = i == 0 ? 'lightblue' : 'darkblue';
